...
def generate_output(prompt):
    ...
    output = client.generate(messages, model="gpt-4o")
    return output.strip()
...
def _api_generate(
    ...
    with Pool(64) as pool:
        responses = list(tqdm(pool.imap(generate_output, queries), total=len(queries)))
    return responses
...
class Evaluator:
    def __init__(
        ...
        if "Llama-Guard-3" in model_name_or_path:
            self.prompter = LlamaGuard3_Prompter(self.template_policy)
        elif "Meta-Llama-3.1-8B-final" in model_name_or_path:
            self.prompter = Safety_Bingo_Prompter(self.template_policy)
        elif "Meta-Llama-3.1-8B-Instruct-v4" in model_name_or_path:
        ...
        elif 'wildguard' in model_name_or_path:
            self.prompter = WildGuard_Prompter(self.template_policy)
        elif 'gpt-4' in model_name_or_path:
            self.prompter = Gpt4_Prompter(self.template_policy)

        # for auprc eval

        # model cache
        self._tokenizer = None
        self._tokenizer = self._load_tokenizer() if not use_api else None
        if self._tokenizer:
            self.safe_begin_token_id = self._tokenizer.encode('safe', add_special_tokens=False)[0]
            self.unsafe_begin_token_id = self._tokenizer.encode('unsafe', add_special_tokens=False)[0]
    ...
    def _hf_first_token(
        ...
        while left < len(queries):
            ...
            if "Llama-Guard-3" in self.model_name_or_path:
                sub_q = [q + '\n' for q in sub_q]
                ...
                for i in range(logits.shape[0]):
                    ...
                    if (
                        ...
                        print(
                            ...
                            f"unsafe_begin_token_id: {tokenizer.decode(max_token_id)}"
    ...
    def _api_generate(
        ...
        with Pool(32) as pool:
            responses = pool.map(partial(generate_output, model=client), queries)
    ...
    def _hf_generate(
        ...
        for prompt in tqdm(queries, desc="Evaluating", disable=not use_tqdm):
            ...
            print(inputs)
            outputs = hf_model.generate(inputs["input_ids"], generation_config=gen_config)
    ...
    def eval_unsafe_probs(
        ...
        """
        self.clear()
        questions = [msg['messages'][0]['content'] for msg in messages]
        answers = [msg['messages'][1]['content'] for msg in messages]
    ...
    def eval_unsafe_labels(
        ...
        if 'wildguard' in self.model_name_or_path and 'phi3' not in self.model_name_or_path:
            resp_list = [extract_label_from_content_for_wildguard(resp) for resp in responses]
            if self.template_policy == 'Agent':
                eval_labels = [label_dict['response'].strip() == 'yes' for label_dict in resp_list]
                resp_cls = [label_dict['prompt'] for label_dict in resp_list]
            else:
                eval_labels = [label_dict['prompt'].strip() == 'yes' for label_dict in resp_list]
                resp_cls = [label_dict['response'] for label_dict in resp_list]
        elif 'gpt-4' in self.model_name_or_path:
            resp_list = [extract_label_from_content_for_api(resp) for resp in responses]
            if self.template_policy == 'Agent':
                eval_labels = [label_dict['response'].strip() == 'yes' for label_dict in resp_list]
                resp_cls = [label_dict['prompt'] for label_dict in resp_list]
            else:
                eval_labels = [label_dict['prompt'].strip() == 'no' for label_dict in resp_list]
                resp_cls = [label_dict['response'] for label_dict in resp_list]
        else:
